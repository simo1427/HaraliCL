...
__kernel void glcmgen_symmetric(__global uchar *img, __global float *res,
                      __global graypair_t *pair, int dx, int dy, int windowsz,
                      int nrows, int ncols) {
    int i = get_global_id(0), j = get_global_id(1);
    int counts = (windowsz - dx) * (windowsz - dy);
    int hws = windowsz / 2;
    ...
    for (i = yrangestart; i < yrangeend; i++) {
        for (j = xrangestart; j < xrangeend; j++) {
            if (i < yend && j < xend && i >= ystart && j >= xstart) {
                for (int imaddr0 = (dy < 0 ? -hws + dy : -hws);
                    ...
                    for (int imaddr1 = (dx < 0 ? -hws + dx : -hws);
                        ...
                        if (tmp == -1) {
                            pair[calculate_address((ncols), dx, dy, windowsz, cnt)].ref = ref;
                            pair[calculate_address((ncols), dx, dy, windowsz, cnt)].val = val;
                            pair[calculate_address((ncols), dx, dy, windowsz, cnt)].weight = 2.0f;
                            cnt++;
                        } else {
                ...
                for (int pairaddr0 = 0; pairaddr0 < cnt + 1; pairaddr0++) {
                    int addr = calculate_address((ncols), dx, dy, windowsz, pairaddr0);
                    sum_private += (pair[addr].weight / (2 * counts)) *
                                                 abs(pair[addr].ref - pair[addr].val);
                }
                res[0 * (ncols - 2 * hws) * (nrows - 2 * hws) +
                        (i - hws) * (ncols - 2 * hws) + (j - hws)] = sum_private;
                ...
                for (int pairaddr0 = 0; pairaddr0 < cnt; pairaddr0++) {
                    int addr = calculate_address((ncols), dx, dy, windowsz, pairaddr0);
                    float tmpres = -(pair[addr].weight / (2 * counts)) *
                                                 log((pair[addr].weight / (2 * counts)));
                    if (isnan(tmpres))
                        continue;
                    sum_private += tmpres;
                }
...
__kernel void glcmgen_asymmetric(__global uchar *img, __global float *res,
                                __global graypair_t *pair, int dx, int dy,
                                int windowsz, int nrows, int ncols) {
    int i = get_global_id(0), j = get_global_id(1);
    int counts = (windowsz - dx) * (windowsz - dy);
            ...
            if (i < yend && j < xend && i >= ystart && j >= xstart) {
                for (int imaddr0 = (dy < 0 ? -hws + dy : -hws);
                    ...
                    for (int imaddr1 = (dx < 0 ? -hws + dx : -hws);
                        ...
                        int val = img[(i + imaddr0 + dy) * ncols + (imaddr1 + dx + j)];
                        int tmp =
                                searchpair(pair, ref, val, (ncols), dx, dy, windowsz, cnt);
                        if (tmp == -1) {
                            pair[calculate_address((ncols), dx, dy, windowsz, cnt)].ref = ref;
                            pair[calculate_address((ncols), dx, dy, windowsz, cnt)].val = val;
                            pair[calculate_address((ncols), dx, dy, windowsz, cnt)].weight = 1.0f;
                            cnt++;
                        } else {
